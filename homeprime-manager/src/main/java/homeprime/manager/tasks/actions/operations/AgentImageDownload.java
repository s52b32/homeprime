...
public class AgentImageDownload implements Callable<Task> {

    private Thing thing;
    private Task task;
    private DownloadData downloadData;

    public AgentImageDownload(Thing thing, Task task, DownloadData downloadData) {
        task.setDescription(AgentImageDownload.class.getSimpleName() + " task for agent " + thing.getUuid() + " "
                + thing.getName());
        this.thing = thing;
        this.task = task;
    ...
    public Task call() {
        ...
        if (AgentRestApiClient.isThingAlive(thing)) {
            ...
            if (thingConfig == null) {
                ...
                // thing is alive, now initiate download
                if (!AgentRestApiClient.downloadActiveImage(thing, downloadData)) {
                    // if terminate request failed to be performed at this point we are sure it failed.
                    task.setStatus(TaskStatusType.Failed);
                    task.setMessage("Failed to download active image to: " + downloadData.getAbsoluteFilePath());
                } else {
                    task.setStatus(TaskStatusType.Completed);
                    task.setMessage("Image succesffully downloaded to: " + downloadData.getAbsoluteFilePath());
                }
            }
        } else {
            task.setStatus(TaskStatusType.Failed);
    ...
    private void checkPrepareDownloadData() {
        if (downloadData == null) {
            ...
            downloadData.setDestinationFileName("image_" + System.currentTimeMillis() + ".zip");
        } else {
