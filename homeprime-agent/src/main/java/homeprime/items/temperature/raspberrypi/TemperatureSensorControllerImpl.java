...
public class TemperatureSensorControllerImpl implements TemperatureSensorController {
	...
	try {
	    ...
	    throw new ThingException("ERROR TemperatureSensorReaderImpl.readI2CTemperature(" + i2cBusNumber + ", " + i2cAddress
		    + ") Unsupported I2C bus selected.", e);
	} catch (IOException e) {
	    throw new ThingException("ERROR TemperatureSensorReaderImpl.readI2CTemperature(" + i2cBusNumber + ", " + i2cAddress
		    + ") Failed to read from I2C device.", e);
	}
    }

    /**
     * Helper method to convert I2C LM75 reading of temperature to decimal
     * number.
     * 
     * @param sensorBuffer
     *            two byte buffer read from sensor
