...
public class TemperatureConfigReader {
	...
	try {
	    if (tempSensors == null) {
		tempSensors = ThingUtils.readFile(ThingProperties.getInstance().getThingConfigPath() + "temperature_sensors.json");
	    }
	    ...
	    throw new ThingException("ERROR ThingTemperatureConfigReader.getTemperatureSensors() Failed to parse temperature_sensors.json", e);
	}
    }
