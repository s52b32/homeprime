...
public class RelayChannelStateControllerImpl implements RelayChannelStateController {
    ...
    public void toggleChannelState(RelayChannel relayChannelData) throws ThingException {
        ...
        final GpioPinDigitalOutput relayChannel = gpio.provisionDigitalOutputPin(RaspiPin.getPinByAddress(relayChannelPin),
                relayChannelData.getName());
        relayChannel.toggle();
        gpio.shutdown();
     ...
     * @param pinAddress
     *        - gpio port number
     * @return {@code true} if pin is set to ON otherwise {@code false}. {@code null} if error
     *         occurs.
     */
