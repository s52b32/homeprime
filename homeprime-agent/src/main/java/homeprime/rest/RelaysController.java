...
public class RelaysController {
    ...
    public ResponseEntity<List<RelayChannel>> getRelayBoardChannels(@PathVariable(value = "relayBoardId") int relayBoardId) {
	try {
	    final RelayBoard findRelayBoardById = findRelayBoardById(relayBoardId);
	    if (findRelayBoardById != null) {
		return new ResponseEntity<List<RelayChannel>>(findRelayBoardById(relayBoardId).getRelayChannels(), HttpStatus.OK);
	    } else {
	...
	try {
		...
		return new ResponseEntity<RelayChannel>(findRelayBoardById(relayBoardId).getRelayChannels().get(relayBoardChannelId), HttpStatus.OK);
	    } else {
	...
	try {
		...
		if (relayChannel != null) {
		    final Boolean currentChannelStatus = RelayChannelStateControllerFactory.getRelayChannelStateReader().readChannelState(
			    relayChannel);
		    return new ResponseEntity<String>(getRelayChannelState(currentChannelStatus), HttpStatus.OK);
		} else {
		    return new ResponseEntity<String>("Relay channel " + relayBoardChannelId + " doesn't exist in relay board " + relayBoardId,
			    HttpStatus.NOT_FOUND);
		}
	    } else {
		return new ResponseEntity<String>("Relay board with id: " + relayBoardId + " not found, cannot proceed with state read",
			HttpStatus.NOT_FOUND);
	    }
	...
	try {
		...
		return new ResponseEntity<String>("Relay board with id: " + relayBoardId + " not found, cannot proceed with toggle",
			HttpStatus.NOT_FOUND);
	    }
	...
	try {
		...
		final RelayChannel relayChannel = findRelayChannelById(findRelayBoard, relayBoardChannelId);
		if (relayChannel != null) {
		    RelayChannelStateControllerFactory.getRelayChannelStateReader().setChannelState(relayChannel, false);
		...
		return new ResponseEntity<String>("Relay board with id: " + relayBoardId + " not found, cannot proceed with power on",
			HttpStatus.NOT_FOUND);
	    }
	...
	try {
		...
		return new ResponseEntity<String>("Relay board with id: " + relayBoardId + " not found, cannot proceed with power off",
			HttpStatus.NOT_FOUND);
	    }
     ...
     * 
     * @param currentRelayChannelStatus
     *            - value of channel status reader
	...
	try {
	    ...
	    throw new ThingException("ERROR ThingRelayController.findRelayBoardById(" + relayBoardId
		    + ") Failed to get relay boards.", e);
	}
